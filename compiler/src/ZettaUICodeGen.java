 ...
 * Produz um arquivo .js compatível com o runtime zettaui.js.
 */
public class ZettaUICodeGen {
    ...
    public String transpile(List<ASTNode> ast, String sourceFile) {
        for (ASTNode node : ast) {
            if (node instanceof ClassDecl) analyzeComponent((ClassDecl) node);
        }
        ...
        for (ASTNode node : ast) {
            if (node instanceof VarDecl)  sb.append(genTopLevelVar((VarDecl) node)).append("\n");
            else if (node instanceof ClassDecl) sb.append(genComponent((ClassDecl) node)).append("\n");
        }
        ...
        if (!componentName.isEmpty()) {
            ...
            sb.append("  ZettaUI._err('render() não retornou um dad.\\nDica: todo componente precisa de um lar.');\n");
            sb.append("}\n");
    ...
    private void analyzeComponent(ClassDecl comp) {
        ...
        if (render != null) collectUILocals(render.body, renderLocals);

        for (FunDecl method : comp.methods) {
            if (method.name.equals("render")) continue;
            if (containsSleep(method.body)) asyncMethods.add(method.name);
            for (String local : renderLocals) {
                if (referencesName(method.body, local)) promoted.add(local);
            }
        }
    }
    ...
    private void collectUILocals(List<ASTNode> body, Set<String> out) {
        for (ASTNode node : body) {
            if (node instanceof VarDecl v && isUIInit(v.init)) out.add(v.name);
        }
    }
    ...
    private String genTopLevelVar(VarDecl v) {
        return "const " + v.name + " = " + genExpr(v.init) + ";";
    }
    ...
    private String genComponent(ClassDecl comp) {
        ...
        for (VarDecl f : comp.fields) {
            sb.append("    this.").append(f.name).append(" = ").append(genExpr(f.init)).append(";\n");
        }
        for (String p : promoted) {
            sb.append("    this._").append(p).append(" = null;\n");
        }
        sb.append("  }\n\n");
        for (FunDecl m : comp.methods) sb.append(genMethod(m)).append("\n");
        sb.append("}\n");
        return sb.toString();
    }
    ...
    private String genMethod(FunDecl m) {
        ...
        if (isRender) collectUILocals(m.body, uiLocals);
    ...
    private String genBody(List<ASTNode> body, boolean inRender, String indent) {
        StringBuilder sb = new StringBuilder();
        for (ASTNode node : body) sb.append(genStmt(node, inRender, indent));
        return sb.toString();
    }
    ...
    private String genStmt(ASTNode node, boolean inRender, String indent) {
        if (node instanceof VarDecl v)  return genVarDecl(v, inRender, indent);
        if (node instanceof ExprStmt es) return genExprStmt(es, indent);
        if (node instanceof ReturnStmt rs) return indent + "return " + genExpr(rs.expr) + ";\n";
        if (node instanceof IfStmt is)  return genIfStmt(is, inRender, indent);
        return indent + "// [stmt: " + node.getClass().getSimpleName() + "]\n";
    }
    ...
    private String genVarDecl(VarDecl v, boolean inRender, String indent) {
        ...
        boolean isUI = isUIInit(v.init);
        String keyword = isUI ? "const" : "let";
        String ref = (inRender && uiLocals.contains(v.name)) ? "_" + v.name : v.name;
        return indent + keyword + " " + ref + " = " + genExpr(v.init) + ";\n";
    }

    private String genExprStmt(ExprStmt es, String indent) {
        if (isSleepCall(es.expr)) return indent + "await " + genExpr(es.expr) + ";\n";
    ...
    private String genExpr(ASTNode node) {
        if (node instanceof Literal l)          return genLiteral(l);
        if (node instanceof Identifier id)      return genIdentifier(id);
        if (node instanceof BinaryExpr b)       return genExpr(b.left) + " " + mapOp(b.op) + " " + genExpr(b.right);
        if (node instanceof AssignExpr a)       return genExpr(a.left) + " = " + genExpr(a.right);
        if (node instanceof MemberAccessExpr m) return genExpr(m.obj) + "." + m.prop;
        if (node instanceof CallExpr call)      return genCall(call);
        return "/* expr? */";
    }
    ...
    private String genCall(CallExpr call) {
        if (call.callee instanceof MemberAccessExpr mac &&
            mac.obj instanceof Identifier id && id.name.equals("ZettaAnim")) {
            return genAnimCall(mac.prop, call.args);
        }

        if (call.callee instanceof MemberAccessExpr mac &&
            mac.prop.equals("mood") && call.args.size() == 1 &&
            call.args.get(0) instanceof Literal l && l.value instanceof String mood &&
            !VALID_MOODS.contains(mood)) {
            System.err.println("✗ Erro: mood(\"" + mood + "\") não reconhecido. Humores válidos: happy, shy, sleepy.");
        }

        if (call.callee instanceof MemberAccessExpr mac &&
            mac.prop.equals("onClick") && call.args.size() == 1) {
            String fn = resolveCallbackArg(call.args.get(0));
    ...
    private String genAnimCall(String method, List<ASTNode> args) {
        ...
        for (int i = 1; i < args.size(); i++) sb.append(", ").append(genExpr(args.get(i)));
        sb.append(")");
        return sb.toString();
    }

    private String resolveCallbackArg(ASTNode arg) {
        if (arg instanceof CallExpr call && call.callee instanceof Identifier id) {
            return "this." + id.name + "()";
        }
        return genExpr(arg);
    }
